...
for layer in range(NUM_LAYERS):
    ...
    for layer_idx in range(len(avg_baseline_logits)):
        ...
        print(f"     - baseline: {avg_baseline_logits[layer_idx]}")
        print(f"     -     ours: {avg_our_logits[layer_idx]}")


# save to CSV
with open(f'/home/scur1188/ai-intepr-project/results/{instrument}_logits_comparison.csv', 'w', newline='') as file:
    writer = csv.writer(file)

    # Write header
    writer.writerow(['layer', 'avg_baseline_logits', 'avg_our_logits'])
